...
module Problem0 {
    ...
    method problem0_1(input: string) returns (x: int)
    {
        ...
        var items := Map((items) => FoldLeft((x,y) => x+y, 0, items), data);
        var sums: seq<int> := MergeSortBy((x,y) => x >= y, items);
        //assume |sums| > 0;
        if |sums| > 0 {
    ...
    {
        var data := parseInput(input);
        var sums: seq<int> := MergeSortBy((x,y) => x >=y, Map((items) => FoldLeft((x,y) => x+y, 0, items), data));
        //assume |sums| > 0;
        if |sums| > 2 {
